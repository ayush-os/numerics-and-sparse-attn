...
From the official Triton docs tutorial (docs.triton-lang.org, 06-fused-attention).
Kernel/autograd code kept as-is; the tutorial's own pytest test_op and
bench_flash_attention harness stripped since this file is only imported for the
`attention` op — this project's own benchmark harness (phase3_kernel_scaffold.py)
does the timing. This is comparison target (b) in Phase 3 (spec.md): the dense
FlashAttention baseline, used unmodified with causal=True. Not the base for the
sparse+quantized kernel — see phase3_kernel_scaffold.py / notes.md for why (this
kernel is prefill-shaped: tiles over BLOCK_M query rows, loops serially over K/V
— not decode-shaped).
"""
